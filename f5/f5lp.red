...
% Essentially, LabeledPolynomial consists of two parts:
...
% Where index of `sgn` is an Integer, and
...
% Constructs a Signature object from index and term
...
% Set the evaluation of lp to ev
...
% Checks if signaturer s1 and s2 are equal
...
% Multiplies signature sgn by the term ev
...
% Mainly, these fall back to simple polynomial coefficients manipulations
% from f5poly
...
% Scales coefficients of the evaluation of `f` by the common denominator
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sorting for LabeledPolynomial

% Returns true if lead(eval(lp1)) < lead(eval(lp2))
...
% Returns true if lead(eval(lp2)) < lead(eval(lp1))
asserted procedure lp_cmpLPLeadReverse(lp1: LabeledPolynomial,
                                        lp2: LabeledPolynomial): Boolean;
  lp_cmpLPLead(lp2, lp1);

% Returns true if total_degree(lead(eval(lp1))) < total_degree(lead(eval(lp2)))

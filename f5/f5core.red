...
% *** Short description of the F5 algorithm with the Rewritten Criterion
% that we implement. This is adapted from the outline given in the work
% by Christian Eder and John Perry
%   https://arxiv.org/pdf/0906.2967.pdf.
%
%   The entry point is the core_groebner1. It receives a list of LabeledPolynomials
% {f1..fm} as an input. core_groebner1 repeatedly invokes core_incrementalBasis
% to construct Groebner bases of ideals {f1,f2}, {f1,f2,f3}..., {f1..fm},
% in succession. LabeledPolynomials are stored in the data structure `r`.
% TODO: Data structure `r` is a vector such as r = [f1,f2,f3]. 
%   The lists Gcurr, Gprev are lists of integers. These integers are the indices
% of the corresponding elements in `r`. Gcurr contains the indices of 
% the generators of {f1..fi}, and Gprev contains the indices of a Groebner basis
% of {f1..fi-1}, respectively.
%   For example, for i = 4, r = [f1,f2,f3,f4], Gprev = {1, 3}, 
% and Gcurr = {1, 3, 4}, the list {f1, f3} is a Groebner basis for {f1,f2,f3}.
% Gcurr corresponds to {f1,f3,f4}, which is the previously computed Groebner basis 
% (stored in Gprev) with f4 adjoined. 
%   The goal of core_incrementalBasis is to compute a Groebner basis of {f1..fi}
% using the previously computed basis from Gprev. It achieves this by computing
% d-Groebner bases for d=0,1,2,... . The core_incrementalBasis iterates the following steps,
% which follow the general outline of Bucherger's algorithm:
...
%   - Reduce polynomials by passing the output of core_computeSpolys to core_reduction.
%   - The output R of core_reduction indexes those polynomials in `r` that did not reduce
% to zero; new critical pairs are generated by iterating core_makeCriticalPair
% on all pairs (k, j) ∈ R × Gcurr, and R is appended to Gcurr.
...
% Where
%   polys is a Vector of `LabeledPolynomial`s,
%   filled is an Integer equal to the number of elements added to `polys`,
%   capacity is an Integer equal to the length of `polys`.
%
% In a single core_groebner1 call, a single ubiquitous Basistracker object is created.
...
% `core_getBasisIdx` is used.
%
% It is safe to assume that polynomials added to the current Basistracker
...
% TODO: core_constructModule is the first thing that is called.
...
asserted procedure core_constructModule(inputBasis: List): List;
  begin scalar outputModule;
    ...
    % polynomials with coprime leading terms after interreduction, if possible.
    ...
    while inputBasis do <<
      ...
      i := i + 1
    >>;
    % if f5statistics is ON, refresh all statistics in the beginning
...
% Computes the normal form of f w.r.t. polynomials r_i where i ∈ Gprev
%
...
%   checks if Term `m` is top-reducible by the previously 
%   computed Groebner basis, which consists of `r_i` for i ∈ Gprev.
%   If it is, returns t;

...
% which is an instance of a `Basistracker` object described below.
%   The lists Gcurr, Gprev are lists of integer index elements of `r`, that
% correspond to the generators of {f1..fi}, and a Groebner basis of {f1..fi-1},
% respectively.
%   The goal of core_incrementalBasis is to compute a Groebner basis of {f1..fi}
% by computing d-Groebner bases for d=1,2... and so on. core_incrementalBasis
...
% all of the pairs of {currIdx} × Gprev
...
%   - The output R of core_reduction indexes those polynomials that did not reduce
...
%     then we return the t flag together with the result of all reductions.
